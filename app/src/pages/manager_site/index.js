...
handleSubmit(event) {
  ...
  const concertsRef = database.ref('-KxJHWYoj6w08GVnyKwz').child('concerts');
  ...
  handleDeclineConcert(artist,key) {
    ...
    database.ref('-KxJHWYoj6w08GVnyKwz').child('artists').child(artist).update({status:"declined"})
  ...
  handleJoinConcert(artist, day, price, key) {
    ...
    database.ref('-KxJHWYoj6w08GVnyKwz').child("concerts").push(data);
    alert("Takk!\n" + artist + " spiller nå på " + day);
    database.ref('-KxJHWYoj6w08GVnyKwz').child("requests").child(key).remove(); //remove from requests
    database.ref('-KxJHWYoj6w08GVnyKwz').child('artists').child(artist).update({status:"booked"}) //setter artist status til booked 
